...
export function ZoneLabel({ title, description, position, hide }: ZoneLabelProps) {
  if (hide) return null;
  return (
    <Html position={position} center distanceFactor={15} zIndexRange={[10, 0]} style={{ pointerEvents: 'none' }}>
      ...
      >
        <div className="bg-brand-paper/95 backdrop-blur-md border border-brand-gold/50 px-4 py-2 shadow-xl flex flex-col items-center gap-1 relative">
          {/* Corner accents */}
          ...
          {description && (
            <span className="font-sans text-brand-dust text-[10px] uppercase font-medium tracking-wider max-w-[150px] text-center opacity-80 pb-0.5">
              {description}
            </span>
          )}
        </div>

        ...
        >
          ...
          <header className="mb-10 md:mb-12 text-center max-w-4xl mx-auto flex flex-col items-center">
            <div className="flex items-center justify-center gap-4 mb-6">
              <span className="text-[10px] uppercase tracking-[0.3em] font-bold text-[#b82736] bg-[#b82736]/10 px-3 py-1 rounded-sm">{dCat || 'Gallery'}</span>
            ...
            <div className="flex items-center gap-4 mt-2">
              <div className="flex flex-col text-center">
                <span className="text-sm font-bold text-brand-ink leading-tight">{dArtist || 'Anonymous Artist'}</span>
                <span className="text-[10px] text-brand-ink/50 uppercase tracking-widest font-bold">Featured Artwork</span>
              </div>
            </div>
          </header>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_8fr_1fr] gap-12 lg:gap-8 max-w-[1100px] mx-auto w-full px-4 md:px-0">
            ...
            <aside className="hidden lg:flex flex-col gap-4 sticky top-32 h-fit items-center text-brand-ink/40 pt-2">
               <span className="text-[9px] uppercase tracking-widest font-bold text-brand-ink/30 mb-2 rotate-180" style={{ writingMode: 'vertical-rl' }}>Share Artwork</span>
               ...
               <button onClick={() => {
                ...
                }} className="w-10 h-10 rounded-full border border-brand-ink/10 flex items-center justify-center hover:bg-brand-ink hover:text-white transition-colors" aria-label="Share">
                 <Share2 size={16}/>
               </button>
            </aside>
            ...
            <div className="prose prose-lg md:prose-xl w-full max-w-2xl mx-auto text-brand-ink/90 font-light leading-[1.8] prose-p:mb-8 prose-strong:font-medium prose-strong:text-brand-ink">
              ...
              {dDesc ? dDesc.split('\n').map((paragraph: string, idx: number) => {
                 ...
                 return <p key={idx} className="mb-8">{text}</p>;
              }) : (
                 <p className="text-center italic text-brand-ink/50 mt-12 mb-12">No detailed description provided for this artwork.</p>
              )}
            </div>
